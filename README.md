...
- Trained and evaluated on the JIGSAWS Suturing dataset (real surgical robot kinematics +
  video).
- Added a YOLO-based detection pipeline (with a classical motion-detection fallback) for
  automated labeling of surgical tool events; it is an independent branch kept for
  downstream analysis and is not wired into generator training.
